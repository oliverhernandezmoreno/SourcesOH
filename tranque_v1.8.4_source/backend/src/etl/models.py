...
def import_executor(name):
    ...
    try:
        ...
        # substring after a colon (':') and may impact the ETL
        # pipeline.
...
def get_executors():
    root = Path(__file__).parent / "executors"
    for candidate in root.glob("**/*.py"):
        candidate_name = ".".join(str(candidate.relative_to(root)).split("/"))[0:-3]
        try:
            executor = import_executor(candidate_name)
            yield executor
            for flavour in executor.flavours:
                yield import_executor(f"{candidate_name}:{flavour}")
        except ETLError:
            pass
