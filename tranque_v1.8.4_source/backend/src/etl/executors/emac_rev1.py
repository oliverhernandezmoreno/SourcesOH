...
class Pipeline(ETLPipeline):
    ...
    error_codes = ETLPipeline.error_codes | StringEnum(
        ...
        INVALID_IONIC_BALANCE="invalid-ionic-balance",
    )

    sample_files = (
        "etl/samples/PlanillaEMACREV1.xlsx",
    )
        ...
        for point_row in operation.extracted_values.filter(
            ...
            if timestamp is None:
                operation.add_error(point_row.make_error(
                    self.error_codes.INVALID_TIMESTMAP,
        ...
        for extracted in operation.extracted_values.filter(
            ...
            if parsed_timestamp is None:
                operation.add_error(extracted.make_error(
                    self.error_codes.INVALID_SOURCE,
            ...
            for index, series in header.items():
                ...
                if previous is not None and previous.value != cleaned.value:
                    operation.add_error(extracted.make_error(
                        self.error_codes.VALUE_MISMATCH,
    ...
    # Dont override conform_source_queryset(self, context, target, groups=None, **_)
    ...
    def validate(self, operation):
        ...
        for source, ib in ionic_balance(operation):
            if ib is None:
                operation.add_error({
                    "code": self.error_codes.INVALID_IONIC_BALANCE,
