...
from targets.models import DataSourceGroup, DataSource, Timeseries, Parameter
...
class ETLPipeline:
    ...
    error_codes = StringEnum(
        PARSING_ERROR="parsing-error",
        OUT_OF_RANGE="out-of-range",
        DELIVERY_ERROR="delivery-error",
    )

    # Override with flavours officially supported by this executor
    flavours = ()

    # Override with the relative paths to the sample files applicable
    # for this executor
    sample_files = ()

    def __init__(self, executor):
        self.executor = executor
        _, *flavour = executor.split(":", 1)
        self.flavour = next(iter(flavour), None)

    def __repr__(self):
        return f"<{self.__class__.__module__}.{self.__class__.__name__}({repr(self.executor)})>"
    ...
    def extract(self, operation):
        ...
        try:
            ...
            operation.add_error(operation.make_error(
                self.error_codes.PARSING_ERROR,
    ...
    # Querysets used for the conform phase

    def conform_group_queryset(self, context, target, **_):
        """Override with custom filtered queryset for DataSourceGroup.

        """
        return DataSourceGroup.objects.filter(target=target)

    def conform_source_queryset(self, context, target, groups=None, **_):
        """Override with custom filtered queryset for DataSource.

        """
        qs = DataSource.objects.filter(target=target)
        if groups is not None:
            qs = qs.filter(groups__in=groups)
        return qs

    def conform_series_queryset(self, context, target, groups=None, sources=None, **_):
        """Override with custom filtered queryset for Timeseries.

        """
        qs = Timeseries.objects.filter(target=target)
        if groups is not None:
            qs = qs.filter(data_source_group__in=groups)
        if sources is not None:
            qs = qs.filter(data_source__in=sources)
        return qs

    def conform_parameter_queryset(self, context, target, **_):
        """Override with custom filtered queryset for Parameter.

        """
        return Parameter.objects.filter(target=target)
    ...
    def deliver(self, operation):
        ...
        try:
            ...
            operation.add_error(operation.make_error(
                self.error_codes.DELIVERY_ERROR,
