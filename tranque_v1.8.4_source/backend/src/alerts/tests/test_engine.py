...
class ModuleSetupAndRunTestCase(ControllerBaseTestCase):
    ...
    def test_run_invokes_the_proper_controllers(self):
        ...
        with target_controllers.collector.override(TESTS_DIR / "fake_trivial_modules"):
            ...
            # check there is a log message for the creation of '.bar' ticket
    ...
    def test_tickets_become_automatically_unevaluable(self):
        with target_controllers.collector.override(TESTS_DIR / "fake_trivial_modules"):
            ...
            bar_module = f"g({self.g1.canonical_name}).bar"
            affected = engine.run(self.target_object, bar_module)
            bar_ticket = Ticket.objects.filter(module=bar_module, state="A").first()
            self.assertIn(bar_ticket, affected)
            # Check there is a log message for the creation of '.bar' ticket
            logs = TicketLog.objects.filter(ticket=bar_ticket)
            self.assertEqual(len(logs), 1)
            # set a data source
            ts = self.timeseries[1][1]
            ts.data_source = self.s1
            ts.save()

            # inactive associated timeseries and re-run the engine
            ts.active = False
            ts.save()
            engine.run(self.target_object, bar_module)
            self.assertTrue(Ticket.objects.filter(pk=bar_ticket.pk, evaluable=False).exists())
            # Check there is a new log for bar ticket
            logs = TicketLog.objects.filter(ticket=bar_ticket)
            self.assertEqual(len(logs), 2)
            self.assertEqual(logs.first().meta['description'], 'evaluable')
            self.assertEqual(logs.first().meta['previous_evaluable'], True)
            self.assertEqual(logs.first().meta['next_evaluable'], False)

            # active the timeseries again re-run the engine
            ts.active = True
            ts.save()
            engine.run(self.target_object, bar_module)
            self.assertTrue(Ticket.objects.filter(pk=bar_ticket.pk, evaluable=True).exists())
            # Check there is a new log for bar ticket
            logs = TicketLog.objects.filter(ticket=bar_ticket)
            self.assertEqual(len(logs), 3)
            self.assertEqual(logs.first().meta['description'], 'evaluable')
            self.assertEqual(logs.first().meta['previous_evaluable'], False)
            self.assertEqual(logs.first().meta['next_evaluable'], True)

            # inactive the asociated data source and re-run the engine
            ts.data_source.active = False
            ts.data_source.save()
            engine.run(self.target_object, bar_module)
            self.assertTrue(Ticket.objects.filter(pk=bar_ticket.pk, evaluable=False).exists())
            # Check there is a new logs for bar ticket
            logs = TicketLog.objects.filter(ticket=bar_ticket)
            ...
            # active the data source again re-run the engine
            ts.data_source.active = True
            ts.data_source.save()
            engine.run(self.target_object, bar_module)
            self.assertTrue(Ticket.objects.filter(pk=bar_ticket.pk, evaluable=True).exists())
            # Check there is a new log for bar ticket
