...
class DeformacionSueloTestCase(EFControllerBase.TestCase):

    def setUp(self):
        super().setUp()
        target = self.target_object
        controller = f'.ef.m2.deformacion_suelo_fundacion.'
        inclinometer = self.inclinometers[0].hardware_id

        self.modules_hints = [
            f's({inclinometer}){controller}B1B4',
            f's({inclinometer}){controller}B2B5',
            f's({inclinometer}){controller}B3B6',
            f's({inclinometer}){controller}C1',
            f's({inclinometer}){controller}C2',
            f's({inclinometer}){controller}C3',
        ]

        self.template_name_x = 'ef-mvp.m2.parameters.deformacion.inclinometro.suelo.eje-x.'
        self.template_name_y = 'ef-mvp.m2.parameters.deformacion.inclinometro.suelo.eje-y.'
        self.template_name_z = 'ef-mvp.m2.parameters.deformacion.inclinometro.suelo.eje-z.'
        ...
        self.base_name = f'{target.canonical_name}.s-{inclinometer}.'
