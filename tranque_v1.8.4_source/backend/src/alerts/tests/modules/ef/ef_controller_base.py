...
class EFControllerBase:
    class TestCase(ControllerBaseTestCase):
        ...
        def setUpTestData(cls):
            ...
            # monoliths group
            cls.monolith_group = DataSourceGroup.objects.create(
                target=target,
                name='monolitos',
                canonical_name='monolitos',
            )
            ...
            cls.monoliths = [
                DataSource.objects.create(
                    target=target,
                    hardware_id=f'test-monolith-{i}-{secrets.token_urlsafe(8)}',
                    name=f'test monolith {i}',
                    canonical_name=f'whatever-{i}-{secrets.token_urlsafe(4)}',
                    type=DataSource.DataSourceType.REFERENCE,
                ) for i in range(3)]
            cls.monolith_group.data_sources.set(cls.monoliths)
            cls.sector_a.data_sources.add(cls.monoliths[0], cls.monoliths[1])
            cls.sector_b.data_sources.add(cls.monoliths[2])

            # inclinometers group
            cls.inclinometer_group = DataSourceGroup.objects.create(
                target=target,
                name='inclinometros',
            ...
            cls.inclinometers = [
                DataSource.objects.create(
                    target=target,
                    hardware_id=f'test-inclinometer-{i}-{secrets.token_urlsafe(8)}',
                    name=f'test inclinometer {i}',
                    canonical_name=f'whatever-{i}-{secrets.token_urlsafe(4)}',
                    type=DataSource.DataSourceType.ONLINE,
                ) for i in range(3)]
            cls.inclinometer_group.data_sources.set(cls.inclinometers)
            cls.sector_a.data_sources.add(cls.inclinometers[0], cls.inclinometers[1])
            cls.sector_b.data_sources.add(cls.inclinometers[2])
