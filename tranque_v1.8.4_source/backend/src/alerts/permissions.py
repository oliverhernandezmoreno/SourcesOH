from alerts.modules.base_states import (
    SEMAPHORE_STATES, ALERT_STATES, EVENT_STATES,
    AUTHORIZATION_LEVELS, CLOSED, ALL_TICKET_STATES,
    DATA_SCOPES
)
...
class AlertsExtraPermissions(ModelExtraPermissions):
    ...
    def permissions(self):
        ...
        # disconnections & manual alert logs
        for scope in DATA_SCOPES:
            yield (
                f'ticket.{scope}.alert_disconnection.read',
                f'Read {scope} alert disconnection '
            )
            yield (
                f'ticket.{scope}.alert_disconnection.write',
                f'Write {scope} alert disconnection '
            )
            yield (
                f'ticket.{scope}.manual_alert.read',
                f'Read {scope} manual alert '
            )
            yield (
                f'ticket.{scope}.manual_alert.write',
                f'Write {scope} manual alert '
            )
