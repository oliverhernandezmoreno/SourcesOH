...
class DataSource(models.Model):
    ...
    sheet = models.ForeignKey(
        Document,
        related_name="datasource_sheets",
        on_delete=models.PROTECT,
...
class Frequency(models.Model):
    ...
    def __str__(self):
        return "".join((
            f"{self.minutes}",
            f" + {self.tolerance_upper}" if self.tolerance_upper is not None else "",
            f" - {self.tolerance_lower}" if self.tolerance_lower is not None else "",
...
class TargetMap(models.Model):
    class TargetMapType:
        choices = (
            ('instruments-map', "Mapa de instrumentos"),
            ('sectors-map', "Esquema de sectores"),
            ('overflow-potential-map', "Esquema de cuenca aportante (potencial de rebalse)")
        )

    id = AutoUUIDField()
    name = models.CharField(max_length=50)
    canonical_name = models.CharField(max_length=100, choices=TargetMapType.choices)
    image = models.ImageField(null=True, blank=True, upload_to="maps/")
    lower_left_coords = ValidatedJSONField(schema=COORDS_SCHEMA)
    upper_right_coords = ValidatedJSONField(schema=COORDS_SCHEMA)
    target = models.ForeignKey(
        Target,
        on_delete=models.PROTECT,
        related_name="maps",
...
class Camera(models.Model):
    ...
    def __str__(self):
        return f'[{str(self.target)}] {self.name}'
...
def video_frame_path(target, camera, filename):
    return '/'.join([
        'target',
        target,
        'camera',
        get_valid_filename(camera),
        filename
    ])


def video_frame_upload_to(instance, filename):
    return video_frame_path(
        instance.camera.target.canonical_name,
...
class VideoFrame(models.Model):
    ...
    image = models.ImageField(upload_to=video_frame_upload_to)
...
@receiver(post_delete, sender=VideoFrame)
