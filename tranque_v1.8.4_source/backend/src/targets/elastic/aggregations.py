...
def top_hits_bucket_to_result(bucket, key):
    """Convert a 'histogram' bucket with a nested 'top_hits' aggregation
    into a result dict.

    """
    timestamp = bucket["key_as_string"]
    value = None
    if len(bucket[key]["hits"]["hits"]) > 0:
        hit = bucket[key]["hits"]["hits"][0]["_source"]
...
def max_aggregation(s, **opts):
    ...
    s.aggs.bucket(
        ...
        "max_value",
        dsl.A("top_hits", sort=[{"value": "desc"}], size=1),
    )
    for bucket in search(s).aggregations.get("histogram", {}).get("buckets", []):
        yield top_hits_bucket_to_result(bucket, "max_value")
...
def min_aggregation(s, **opts):
    ...
    s.aggs.bucket(
        ...
        "min_value",
        dsl.A("top_hits", sort=[{"value": "asc"}], size=1),
    )
    for bucket in search(s).aggregations.get("histogram", {}).get("buckets", []):
        yield top_hits_bucket_to_result(bucket, "min_value")
...
def last_aggregation(s, **opts):
    ...
    for bucket in search(s).aggregations.get("histogram", {}).get("buckets", []):
        yield top_hits_bucket_to_result(bucket, "last_value")
