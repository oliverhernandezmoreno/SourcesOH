...
class BaseTestCase(test.APITestCase):
    ...
    def make_file(self, suffix):
        path = str(Path(settings.MEDIA_ROOT) / f'{secrets.token_urlsafe(8)}{suffix}')
        fd = default_storage.open(path, mode='w')
        fd.write(' ')  # write a character to force creation if storage is S3
        fd.close()
        self.files = [*getattr(self, "files", ()), path]
        logger.info(f"Created random file {path}")
        return path
...
class TestRunner(DiscoverRunner):
    ...
    def setup_test_environment(self, **kwargs):
        ...
        test_media_root = str(self._tmp_media)
        if settings.DEFAULT_FILE_STORAGE == "storages.backends.s3boto3.S3Boto3Storage":
            # remove leading slash from test_media_root to avoid conflicts with s3 storage
            test_media_root = test_media_root[1:]
        settings.MEDIA_ROOT = test_media_root
        logger.info(f"MEDIA_ROOT set to {settings.MEDIA_ROOT} during tests")
        logger.info(f"DEFAULT_FILE_STORAGE is {settings.DEFAULT_FILE_STORAGE} during tests")
    ...
    def setup_databases(self, *args, **kwargs):
        ...
        if not self.keepdb or Target.objects.count() == 0:
            # if db has any target, implies test db setup was already run before
            # so if keepdb is true ignore initial setup to speed up local testing
            baseSetUp()
        create_remote()
