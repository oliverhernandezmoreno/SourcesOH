...
from api.v1.handlers.utils import (
    get_events, get_events_related_data, get_data_source_folder,
)
...
from remotes.storage import get_target_bucket
...
def get_ticket_bucket(ticket):
    return get_target_bucket(ticket.target)
...
def prepare_ticket_message_body(ticket):
    ...
    for source in [s for s in sources if s.sheet is not None]:
        folder = get_data_source_folder(source)
        documents.extend(serialize_docs_for_upload([source.sheet], folder))
