...
from alerts.modules.base_states import EVENT_STATES, SEMAPHORE_STATES, AUTHORIZATION_LEVELS, ALERT_STATES, CLOSED
...
class TicketPermissionsTestCase(BaseTestCase):
    ...
    def ticket_read_permission_test(self, ticket, second_ticket, state):
        ...
        # if state in SEMAPHORE_STATES or state == CLOSED:
        if state in SEMAPHORE_STATES:
    ...
    def create_ticket_test(self, new_state):
        # Intent
        intent_url = self.api_url('target', self.target, 'status', '_.ef.alert', 'intent')
        intent_content = {'state': new_state}
        perm = f'ticket.{new_state}.create'
        ...
        try:
            with transaction.atomic():
                ...
                new_ticket = data['tickets'][0]
                self.assertEqual(new_ticket['state'], new_state)
                raise RollbackException()
        except RollbackException:
            pass

        self.mark_as_checked(perm)
