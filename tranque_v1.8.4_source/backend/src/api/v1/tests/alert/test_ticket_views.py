...
from alerts.models import Ticket, TicketLog, TicketComment, Broadcast, AuthorizationRequest
...
class TicketViewTestCase(BaseTestCase):
    ...
    def setUp(self):
        ...
        for index, (module, controller) in enumerate(self.controllers.items()):
            ticket = Ticket.objects.create(
                module=module,
                state=list(controller.states)[index % 6],
    ...
    def test_approve_authorization_smc_request(self, mock_upload, docs):
        ...
        for call in mock_upload.call_args_list:
            ...
            self.assertIn(call[0][0].name.split('/')[-1], doc_names)
    ...
    @with_fake_docs(count=2)
    def assertComment(self, comment_type, ticket, docs):
        ticket.evaluable = True
        ticket.save()
        engine.run(self.target_object, [ticket.module])
        ticket.refresh_from_db()
        commentContent = "Comment content ..."
        response = self.client_post(
            self.api_url(ticket.pk, "comment"),
            {
        ...
        self.assertEqual(comment.content, commentContent)
        self.assertEqual(comment.documents.count(), 2)
        commentLog = TicketLog.objects.filter(ticket=ticket).first()
        self.assertEqual(commentLog.meta["comment_id"], comment.id)
        self.assertEqual(commentLog.meta["comment_type"], comment_type)
        self.assertEqual(commentLog.meta["description"], "comment")
        self.assertEqual(commentLog.meta["comment"], commentContent)
        self.assertEqual(commentLog.documents.count(), 2)

    @with_test_modules
    def test_ticket_create_comment(self):
        with self.subTest("event management comment"):
            ticket = Ticket.objects.filter(
                reduce(lambda x, y: x | y, [Q(state=state) for state in EVENT_STATES])
            ).first()
            self.assertComment("event_management", ticket)
        with self.subTest("alert complementary comment"):
            ticket = Ticket.objects.filter(state=ALERT_STATES.RED).first()
            self.assertComment("alert_complementary", ticket)
        with self.subTest("alert management comment"):
            ticket = Ticket.objects.filter(state=ALERT_STATES.RED).first()
            self.assertComment("alert_management", ticket)
        with self.subTest("close report comment"):
            ticket = Ticket.objects.filter(state=ALERT_STATES.RED).first()
            self.assertComment("close_report", ticket)

    @with_test_modules
    @with_fake_docs(count=1)
    def test_ticket_download_comment_document(self, docs):
        ticket = self.tickets[0]
        comment = TicketComment.objects.create(
            ticket=ticket,
            comment_type="event_management"
        )
        comment.documents.add(docs[0])
        comment.save()
        response = self.client_get(
            self.api_url(ticket.id, "comment", comment.id, "document", docs[0].id, "download")
        )
        self.assertResponseOk(response)
