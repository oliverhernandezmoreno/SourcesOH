...
from api.v1.views.timeseries_views import NestedTimeseriesDocumentView
    ...
    def test_retrieve_timeseries_aggregation(self):
        ...
        with self.subTest('test min aggregation'):
            with self.subTest('test min with default params'):
                ...
                self.assertEqual(results[2]['@timestamp'], '2017-05-18T08:00:00.000Z')
                self.assertIsNone(results[2]['value'])
                self.assertEqual(results[3]['@timestamp'], '2017-05-18T16:20:00.000Z')
...
class TimeseriesDocumentTestCase(BaseTestCase):

    @contextlib.contextmanager
    def random_file(self):
        "Yields the filename and a django random File"
        path = self.make_file(".bin")
        with default_storage.open(path, "wb") as f:
            f.write(secrets.token_bytes(32))
        with default_storage.open(path, "rb") as f:
            yield Path(path).name, File(f)

    def setUp(self):
        # Become a mine user
        self.as_mine()
        # Create a timeseries, document pair
        self.timeseries = Timeseries.objects.create(
            target=self.target_object,
            canonical_name=f"test-document-{secrets.token_urlsafe(4)}",
        )

        # create documents with fixed timestamps
        years = [2014, 2015, 2016, 2017]
        self.documents = []
        for year in years:
            date = datetime.datetime(year, 1, 1)
            with self.random_file() as (filename, f):
                document = Document.objects.create(
                    folder=NestedTimeseriesDocumentView.folder_for_timeseries(self.timeseries),
                    file=f,
                    name=filename
                )
                self.documents.append(document)
                Document.objects.filter(pk=document.pk).update(created_at=date)

        # set document type on the first two
        self.document_type = DocumentType.objects.create(
            id="doctype1",
            description="this is a document type example"
        )

        for document in self.documents[:2]:
            Document.objects.filter(pk=document.pk).update(type=self.document_type.id)

        # assign them to the timeseries
        self.timeseries.documents.set(self.documents)

        # Create another pair of timeseries, document to test for
        # appropriate filters
        self.other_timeseries = Timeseries.objects.create(
            target=self.target_object,
            canonical_name=f"test-other-{secrets.token_urlsafe(4)}",
        )
        with self.random_file() as (filename, f):
            self.other_document = Document.objects.create(
                folder=NestedTimeseriesDocumentView.folder_for_timeseries(self.other_timeseries),
                file=f,
                name=filename
            )
        self.other_timeseries.documents.set([self.other_document])

    def test_read_list(self):
        with self.subTest("list shows the timeseries' documents"):
            url = reverse(
                f'{self.api_version}:target-timeseries-document-list',
                args=[self.target, self.timeseries.canonical_name]
            )
            response = self.client_get(url)
            self.assertResponseOk(response)
            results = response.data["results"]
            ...
            expected_ids = [d.id for d in self.documents]
            self.assertEqual(set(received_ids), set(expected_ids))

        with self.subTest("read finds a specific document"):
            url = reverse(
                f'{self.api_version}:target-timeseries-document-detail',
                args=[self.target, self.timeseries.canonical_name, self.documents[0].id]
            )
            response = self.client_get(url)
            self.assertResponseOk(response)

        with self.subTest("read doesn't find document's of other timeseries"):
            url = reverse(
                f'{self.api_version}:target-timeseries-document-detail',
                args=[self.target, self.other_timeseries.canonical_name, self.documents[0].id]
            )
            response = self.client_get(url)
            self.assertResponseStatus(404, response)

        with self.subTest("list shows the timeseries' documents created within two dates"):
            url = reverse(
                f'{self.api_version}:target-timeseries-document-list',
                args=[self.target, self.timeseries.canonical_name]
            )
            ...
            response = self.client_get(url, {
                'created_at_from': '2014-01-01',
                'created_at_to': '2016-02-01'
            })
            self.assertResponseOk(response)
            results = response.data["results"]
            self.assertEqual(len(results), 3)

            expected_years = [2016, 2015, 2014]
            for year, result in zip(expected_years, results):
                self.assertEqual(result.get('created_at'), f"{year}-01-01T00:00:00Z")

        with self.subTest("list shows the timeseries' documents of one type only"):
            url = reverse(
                f'{self.api_version}:target-timeseries-document-list',
                args=[self.target, self.timeseries.canonical_name]
            )
            ...
            results = response.data["results"]
            self.assertEqual(len(results), 2)
            expected_years = [2015, 2014]
            for year, result in zip(expected_years, results):
                self.assertEqual(result.get('created_at'), f"{year}-01-01T00:00:00Z")
                self.assertEqual(result.get('type'), "doctype1")

    def test_upload(self):
        with self.subTest("upload a file without type"):
            url = reverse(
                f'{self.api_version}:target-timeseries-document-upload',
                args=[self.target, self.timeseries.canonical_name]
            )
            with self.random_file() as (filename, f):
            ...
            list_response = self.client_get(list_url)
            self.assertResponseOk(list_response)
            results = list_response.data["results"]
            self.assertEqual(len(results), 5)
            self.assertEqual(results[0]["id"], response.data["id"])

        with self.subTest("upload a file with a type"):
            url = reverse(
                f'{self.api_version}:target-timeseries-document-upload',
                args=[self.target, self.timeseries.canonical_name]
            )
            with self.random_file() as (filename, f):
                response = self.client_post(
                    url,
                    {"file": f, "filename": filename, "type": "doctype1"},
                    format="multipart",
                )
                self.assertResponseOk(response)
                list_url = reverse(
                ...
                results = list_response.data["results"]
                received_names = [r['name'] for r in results]
                self.assertIn(filename, received_names)

                new_document = next(filter(lambda r: r['name'] == filename, results), None)
                self.assertEqual(new_document['type'], "doctype1")

        with self.subTest("upload a file with a wrong type"):
            url = reverse(
                f'{self.api_version}:target-timeseries-document-upload',
                args=[self.target, self.timeseries.canonical_name]
            )
            with self.random_file() as (filename, f):
                response = self.client_post(
                    url,
                    {"file": f, "filename": filename, "type": "wrong type"},
                    format="multipart",
                )
                self.assertResponseStatus(status.HTTP_400_BAD_REQUEST, response)
                self.assertEqual(response.data, 'Wrong document type.')

    def test_download(self):
        url = reverse(
            f'{self.api_version}:target-timeseries-document-download',
            args=[self.target, self.timeseries.canonical_name, self.documents[0].id]
        )
        response = self.client_get(url)
        self.assertResponseOk(response)
