...
/**
 * Fetches the structure of an ETL executor
 */
export const readExecutor = (options) => {
    const opts = {
        cache: null,
        host: config.API_HOST,
        target: null,
    ...
    const url = `${opts.host}/v1/target/${opts.target}/etl-executor/${opts.executor}/`;
    return HttpClient.get(url, {
        cache: opts.cache
    }).pipe(mergeMap((r) => of(r.data)));
};
...
/**
 * Factory which generates a fetcher for a resource required by an
 * executor.
 */
const makeExecutorResourceFetcher = (resource) => (params) => handleListPagination(params, (options) => {
    const opts = {
        cache: null,
        host: config.API_HOST,
        page: null,
    ...
    const url = `${opts.host}/v1/target/${opts.target}/etl-executor/${opts.executor}/${resource}/`;
    return HttpClient.get(url, {
        cache: opts.cache,
        params: {
            ...(opts.page !== null ? {page: opts.page} : {}),
...
/**
 * Fetches the groups required by an executor.
 */
export const listExecutorGroups = makeExecutorResourceFetcher('group');

/**
 * Fetches the sources required by an executor.
 */
export const listExecutorSources = makeExecutorResourceFetcher('source');

/**
 * Fetches the series required by an executor.
 */
export const listExecutorSeries = makeExecutorResourceFetcher('series');

/**
 * Fetches the parameters required by an executor.
 */
export const listExecutorParameters = makeExecutorResourceFetcher('parameter');
