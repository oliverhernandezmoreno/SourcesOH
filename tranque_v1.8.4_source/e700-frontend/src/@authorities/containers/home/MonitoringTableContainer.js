...
export const MonitoringTableContainer = withStyles(styles)(class extends SubscribedComponent {
    ...
    getInitialTickets() {
        ...
        this.subscribe(
            ...
            () => this.getInitialDisconnections()
        );
    }

    /**
     * Function triggered to get disconnections from alert disconnection end-point.
     *
     * @public
     */
    getInitialDisconnections() {
        this.setState({
            initialLoading: true,
            initialLoadText: 'desconexiones...',
            loadError: false
        });
        this.subscribe(
            DisconnectionsService.listAll(),
            data => {
                this.setState(state => ({
                    disconnections: data,
                    initialLoading: false,
        ...
        this.subscribe(
            ...
            ({total, page, data}) => {
                this.setState(state => {
                    ...
                    if (page === 1) {
                        ret.tableData = generateTableData(targetArray, [], state.tickets, state.disconnections, availability);
                    }
                    return ret;
                });
            },
    ...
    getInitialIndexes() {
        ...
        this.subscribe(
            ...
            data => {
                this.setState(state => ({
                    indexes: data,
                    tableData: generateTableData(Object.values(state.targets), data, state.tickets, state.disconnections, availability),
                    initialLoading: false,
                    lastUpdate: moment().format(config.DATETIME_FORMAT)
                }));
                this.setDataUpdate();
            ...
            ({tickets, targets, timeseries, requests_data}) => {
                this.setState(state => {
                    ...
                    return {
                        loading: false,
                        lastUpdate: moment().format(config.DATETIME_FORMAT),
                        tableData: generateTableData(Object.values(_targets), timeseries, state.tickets, state.disconnections, availability),
                        tickets: tickets,
                        indexes: timeseries,
                        targets: _targets,
    ...
    render() {
        const {initialLoading, loading, lastUpdate, tableData, tickets,
            zoneInfo, initialLoadText, loadError, disconnections, requests, siteParameters} = this.state;
        ...
        return (
            <div className={classes.container}>
                ...
                <MonitoringAlertsEvents data={tickets} requests={requests} disconnections={disconnections}/>
