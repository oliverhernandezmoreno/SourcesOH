...
class TargetMainClass extends SubscribedComponent {
    ...
    loadData() {
        ...
        this.subscribe(
            forkJoin({
                target: TargetService.get({canonical_name: target}),
                cameras: CameraService.listAll({target: target})
            }),
            ({target, cameras}) => {
                this.setState({loadingData: false, target, cameras});
                cameras.forEach(cam => this.getVideoFrameBlob(cam.id, cam.video_frames[0].id));
            }
        );
    }
    ...
    render() {
        ...
        const {target, cameras, videoFrames} = this.state;
