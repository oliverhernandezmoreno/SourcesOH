...
class DescriptionSection extends Component {
    ...
    getDescription() {
        ...
        const message = ticket.result_state.message;
        const adviceIndex = message.indexOf('Recomendación:');
        if (adviceIndex > 0) {
            const definition = message.substring(0, adviceIndex);
            const adviceTitle = message.substring(adviceIndex, adviceIndex + 14);
            const advice = message.substring(adviceIndex + 14, message.length);
            return (<>{definition}<b>{adviceTitle}</b>{advice}</>);
        }
        else return message;
    }
