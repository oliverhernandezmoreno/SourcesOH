...
import {OPEN, NEW_SENT, ANSWER_RECEIVED,
    ANSWER_REVIEWED, NEW_SENDING} from '@e700/constants/reportFormStates';
...
export const FormRegistry = withStyles(styles)(class extends SubscribedComponent {
    ...
    itemPredicate(item, listFilter) {
        switch (listFilter) {
            ...
            case 'reviewed-with-comments':
                return (
                    item && item.state === ANSWER_REVIEWED &&
    ...
    instanceToTableRow(instance) {
        ...
        let entity = '--';
        let faena = '--';
        if (instance && instance.work_sites.length > 0) {
        ...
        return {
            ...
            button: <Button
                style={{width: '100px', whiteSpace: 'nowrap', color: '#ffffff', backgroundColor: '#1A76D1'}}
                variant="contained"
    ...
    renderTable(instances) {
        const {loading, classes, listFilter} = this.props;
        if (loading) {
            ...
            const tableColumns = [
                {
                    ...
                    render:(data) => <Checkbox
                        checked={
                            this.state.checkbox_array && this.state.checkbox_array.length > 0 &&
                ...
                listFilter === 'all' ? {
                    title: <Typography variant="body2">Estado del formulario</Typography>,
                    field: 'formState',
                    render: data => this.translateState(data.formState),
                    searchable: false
                }: null,
                {
                    title: <Typography variant="body2">Empresa/Faena</Typography>,
                    field: 'workSite',
                    cellStyle: {width: 10, maxWidth: 10, wordWrap: 'break-word'},
            ...
            const tableData = instances.filter((instance) => {
                return ((this.state.selectedYear !== '' && this.state.selectedYear === instance.year) ||
                    ...
                    this.state.selectedTrimester === instance.trimester) || this.state.selectedTrimester === '');
