...
class InspectStepper extends SubscribedComponent{
  ...
  handleAnswer = (items, answers) => {
      ...
      const arrayAnswers = events;
      items.forEach(item => {
          if (answers[item.canonical_name]!==undefined)
          {arrayAnswers.push({
              name: item.canonical_name, value: findChoice(item.choices, +answers[item.canonical_name]).value.choiceValue})
          }
      });
  ...
  handleChangeEvents = (group,name) => event => {
      if(group === "m1-triggers"){
          const triggers = {...this.props.triggers};
          triggers.answersTriggersEvents[name]= event.target.value;
          this.setState({triggers:triggers})
      }
      else if(group === "m1-critical-triggers"){
          const triggers = {...this.props.triggers};
  ...
  render(){
      ...
      return(
          <div className={classes.root} >
              ...
              <Paper style={{margin:"20px", marginTop:"10px"}}>
