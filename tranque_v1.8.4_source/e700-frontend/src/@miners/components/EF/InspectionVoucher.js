...
// import VulnerabilityResult from '@miners/components/EF/VulnerabilityResult';
...
class Voucher extends SubscribedComponent {
  ...
  loadData = () => {
      ...
      this.subscribe(forkJoin({
          ...
          resultVulnerability: TimeseriesService.list({
              cache: 60 * 1000,  // one minute
              target: target,
              template_name: 'ef-mvp.m1.vulnerability',
              max_events: 1
          }),
      }),
      ({operationObject, data, resultVulnerability}) => this.subscribe(
  ...
  renderMonthlyTables() {
      ...
      // const {resultVulnerability} = this.state;
          ...
          <div>
              ...
              <div className={classes.footer}>
                  <div className={classes.subFooter}>
                      <Button
                          variant="contained"
                          onClick={() => history.push(reverse(`miners.target.ef.inspection-and-evaluation.registry`, {
                              target
                          }))}
