...
class OperationsList extends SubscribedComponent {
    ...
    componentDidUpdate(prevProps) {
        if (
            prevProps.executors.some(
                (e) => !this.props.executors.some(({executor}) => e.executor === executor)
            ) ||
            this.props.executors.some(
                (e) => !prevProps.executors.some(({executor}) => e.executor === executor)
            )
        ) {
    ...
    etlTemplate(name) {
        return (
            this.props.executors.find(({executor}) => name === executor) || {
                title: name,
            }
        ).title;
    }
        ...
        if (operation.state === 'success') {
            ...
            return (
                <Grid container direction="row" alignItems="center">
                    ...
                    <Grid item>
                        <Typography className={classes.errorText}>Con errores</Typography>
                    </Grid>
                </Grid>
            );
        }
            ...
            <Table>
                <TableHead>
                    <TableRow>
                        ...
                        <TableCell component="th">Inicio</TableCell>
                        <TableCell component="th">Plantilla</TableCell>
                        <TableCell component="th">Usuario</TableCell>
                        <TableCell component="th">Datos precargados</TableCell>
                        <TableCell component="th">Estado</TableCell>
                        <TableCell component="th" align="right">
                            <IconButton
                                disabled={
                                    this.state.page * this.state.pageSize >= this.state.operations.count
                                }
                                onClick={this.setPage(this.state.page + 1)}
                            >
                                <ChevronRight />
                ...
                <TableBody>
                    {this.state.operations.results.map((operation, index) => (
                        <TableRow key={operation.id}>
                            ...
                            <TableCell>
                                <Typography>{this.etlTemplate(operation.executor)}</Typography>
                            </TableCell>
                            <TableCell>
                            ...
                            <TableCell>{this.renderStatusIcon(operation)}</TableCell>
                            <TableCell>
