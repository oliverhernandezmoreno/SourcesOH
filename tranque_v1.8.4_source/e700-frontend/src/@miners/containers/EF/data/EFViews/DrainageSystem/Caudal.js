...
class Caudal extends SubscribedComponent {
    ...
    loadListSeries() {
        ...
        // ef-mvp.m2.parameters.integridad-sistema-drenaje.falla-sistema-drenaje

        this.loadSwitchData('integridad-sistema-drenaje.falla-sistema-drenaje').then(drainageFailureData => {
            this.setState({
                drainageFailureData
            })
        });
    ...
    render() {
        ...
        const drainageFailureData = this.state.drainageFailureData?.[0];
        return (
            <div>
                    ...
                    <div className={classes.content}>
                        {
                            plotTemplates.map(plotTemplate => {
                                 ...
                                 */
                                const getRowGroupLabel = (row) => row?.data_source?.group_names?.[2] ?? '';
                        ...
                        <div className={classes.content__details} >
                            <div className={classes.details__section}>
                                ...
                                <SwitchBox
                                    header='Falla en el sistema de drenaje'
                                    bodyContent='Se detectó una falla en el sistema de drenaje'
                                    // checked={true}
                                    switchProps={{
                                        checked: drainageFailureData?.events?.[0]?.value === 1,
                                        disabled: drainageFailureData?.events?.[0]?.value === undefined || this.props.disableActions
                                    }}
...
export default connect(MapStateToProps, null)(withStyles(styles)(Caudal));
