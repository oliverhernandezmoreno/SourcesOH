...
resource "azurerm_key_vault" "key" {
 ...
 data "azurerm_key_vault_secret" "alfa" {
  name         = azurerm_key_vault_certificate.alfa.name
  key_vault_id = azurerm_key_vault.key.id
 }
...
resource "azurerm_key_vault_certificate" "minio" {
  ...
  certificate {
    contents = filebase64("./cert/pem/minio.inriadev.cl/cert-minio.pfx")
    password = ""
  }
...
resource "azurerm_key_vault_certificate" "w2" {
  ...
  certificate {
    contents = filebase64("./cert/pem/w2.inriadev.cl/cert-w2.pfx")
    password = ""
  }
...
resource "azurerm_key_vault_certificate" "alfa" {
  name         = "site-principal"
  key_vault_id = azurerm_key_vault.key.id

  certificate {
    contents = filebase64("./cert/pem/alfa.inriadev.cl/cert-alfa.pfx")
    password = ""
  }
...
resource "azurerm_key_vault_certificate" "static" {
  ...
  certificate {
    contents = filebase64("./cert/pem/static.inriadev.cl/cert-static.pfx")
    password = ""
  }
