...
const refMax = await refs.getMany("*.deformacion-inclinometro-z-eje-*");


if (utils.isDefined(events) &&
  utils.isDefined(events[0]) &&
  utils.isDefined(refMax) &&
  utils.isDefined(eventsRef)) {
  ...
  const superaciones = refMax
    ...
    .filter(({threshold, event}) => utils.isDefined(event) && event.value > Math.min(
  ...
  // Supera umbral
  series.yield(~~(superaciones.length > 0), {meta: {
    superaciones
  }});
}
