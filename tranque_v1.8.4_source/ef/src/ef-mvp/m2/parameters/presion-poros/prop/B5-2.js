const {meta: {vars, heads}} = utils.assert(await series.query({head: "*.t2.*"}));
...
const {meta: {vars: t1Vars, heads: t1Heads}} = utils.assert(await series.query({head: "*.t1.*"}));
const otherVars = t1Vars.filter(
  ({data_source: {groups}}) => groups.some((g) => g.startsWith("ubicacion-") && g !== location)
);
const others = otherVars
      .map(({canonical_name}) => t1Heads.find(({name}) => name === canonical_name))
