...
test("raises if at least two redundant events are raised, and one of them is valid", async () => {
  // setup
...
test("raises if at least two redundant events are raised, and the current one is valid", async () => {
  // setup
  seriesQueryMock.mockReturnValueOnce(Promise.resolve({meta: {
    vars: [
      {data_source: {id: "this-source"}, canonical_name: "this-variable"},
...
test("doesn't raise events if no events are valid", async () => {
  // setup
  seriesQueryMock.mockReturnValueOnce(Promise.resolve({meta: {
    vars: [
      {data_source: {id: "this-source"}, canonical_name: "this-variable"},
