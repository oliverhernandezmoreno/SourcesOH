 ...
 * - executors[].sample: the path to a downloadable sample
...
class Stepper extends SubscribedComponent {
    ...
    updateOperation(operationObject) {
        this.setState({
            ...
            operationObject,
            loadError: operationObject === null
        });
    }

    loadOperation() {
        this.subscribe(
            etlService.read({target: this.props.target, operation: this.props.operation}),
        ...
        if (this.state.activeStep === 0) {
            return (
                <div className={classes.stepBody}>
                    {React.createElement(spec.loader, {target, executor, sample: spec.sample, spec})}
                </div>
            );
        }
    ...
    render() {
        ...
        const spec = executors.find(
            (s) => this.state.operationObject ?
                ...
                return (
                    <>
                        <Typography variant="h6">
                            No se pueden cargar datos con la plantilla especificada
                        </Typography>
                        <Typography color="error"><code>{executor}</code></Typography>
                        <Typography>
                            Use una plantilla soportada por la plataforma seleccionando desde el menú.
                        </Typography>
                    </>
                );
            } else {
                return (
                    this.state.operationObject === null ?
                        this.renderLoading() :
                        <DefaultVoucher operation={this.state.operationObject}/>
                );
            }
        }
        return (
            <>
                <MaterialStepper activeStep={this.state.activeStep} alternativeLabel className={this.props.classes.stepper}>
