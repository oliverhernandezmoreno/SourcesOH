...
const DefaultVoucher = ({operation, auth, children}) => {
    const success = operation.state === 'success';
    let errorLabel = 'La plantilla de carga ya no está soportada por el sistema';
    ...
    return (
        <Card>
            <CardHeader avatar={<Avatar>{success ? <Done/> : <Error/>}</Avatar>}
                title={`Registro de ${success ? '' : 'intento de '}carga ${success ? 'exitosa ' : ''}de datos`}
...
export default connect(mapStateToProps)(DefaultVoucher);
