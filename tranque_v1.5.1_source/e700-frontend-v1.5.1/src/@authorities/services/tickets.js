 ...
 */
export function hasActiveDisconnection(disconnectionlist, profile) {
    const profileactivedisconnections = disconnectionlist ? disconnectionlist.filter(d => d.scope === profile) : [];

    if (profileactivedisconnections.length === 0) return false;
    
    let hasActiveDisconnection = false;
    profileactivedisconnections.forEach(d => {
        if (!d.closed) hasActiveDisconnection = true;
    });

    return hasActiveDisconnection;
}