...
class RequestsBox extends Component {
    ...
    renderRequestsBox(status){
        const {classes, handleRequest, waitingDumps} = this.props;
        ...
        const boxContent = [
            {
                ...
                infoText: (<Box>Dentro del rango de fechas seleccionadas, existen&nbsp;
                    ...
                    {/* <BlueButton className={classes.requestButton} variant="contained" color="primary" disableElevation>SOLICITAR DATOS</BlueButton> */}
                </Box>)
            },
            {
                state: 1,
        ...
        const selectedBoxContent = boxContent.find((c) => c.state === dumpRequestsStatus ? c : false);
    ...
    renderRequestsDialog() {
        ...
        const {openRequestsDialog, dataGaps} = this.state;
        return (
    ...
    componentDidMount(){
        const {dataGaps, failedDump} = this.props;
        if (failedDump){
            this.setState({
                dumpRequestsStatus: -1,
                dataGaps: [],
    ...
    render() {
        ...
        return (<>
            {openRequestsBox ? [this.renderRequestsBox(), this.renderRequestsDialog(), this.renderTimeConditionsDialog()] : null}
        </>);
    }
}
