...
AMQPPASS=$(cat ./sml-1.secrets | grep "^AMQPPASS=" | sed -e "s/^AMQPPASS=//g")

find stack -type f | while read -r templatefile ; do
    sed -i \
        -e "s/__REPLACED_TARGET__/el-mauro/g" \
        -e "s/__REPLACED_DOMAIN__/staging-sml-1.observatorioderelaves.cl/g" \
        -e "s/__REPLACED_NAMESPACE__/staging-sml-1/g" \
        -e "s/__REPLACED_BUCKET__/sml-1/g" \
        -e "s/__REPLACED_AMQPUSER__/sml-1/g" \
        -e "s/__REPLACED_AMQPPASS__/${AMQPPASS}/g" \
        -e "s/__REPLACED_USERS_FILE__/users-sml-1.secrets/g" \
        "${templatefile}"
done
